...
class Reader(object):
    ...
    def __init__(self, data):
        # ``data`` might be a bytes object or a string.  ``io.open`` under
        # Python 3 returns bytes while older versions of this script used
        # strings.  Normalise to bytes so the parsing logic works on both
        # versions of Python.
    ...
    def byte(self):
        """Return the next byte as a one-character string."""
        self.pos += 1
        val = self.data[self.pos - 1]
        if isinstance(val, int):  # Python 3 returns integers when indexing bytes
            return chr(val)
        return val
    ...
    def string(self):
        length = self.uint16()
        self.pos += length
        # Slice returns ``bytes``; decode so callers always get a string
    ...
    def getXString(self, size):
        self.pos += size
        return self.data[self.pos - size:self.pos].decode('latin-1')
...
class Node(object):
    __slots__ = ('data', 'nodes', 'begin', 'size')
