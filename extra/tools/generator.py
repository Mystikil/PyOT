...
ITEMS_FILE = os.path.join(HERE, "..", "..", "data", "items.json")
parse = json.load(io.open(ITEMS_FILE, 'r'))
for item in parse:
                    ...
                    for row in range(areas[1]):
                        ...
                        if y:
                            ...
                            if pos in self.houses:
                                y.append(HiFormat.pack(50, self.houses[pos]).decode('latin-1'))
                            ...
                            if pos in self.flags:
                                y.append(HiFormat.pack(51, self.flags[pos]).decode('latin-1'))
...
class Item(object):
    ...
    def writeAttribute(self, name, value):
        ...
        # A number?
            ...
            string += "i" + struct.pack("<i", value).decode('latin-1')
            
        # A string?
        elif isinstance(value, str):
            string += "s" + struct.pack("<i", len(value)).decode('latin-1') + value
    ...
    def gen(self, x,y,z,rx,ry,extras):
        ...
        code = struct.pack("<H", self.id).decode('latin-1')
...
class Spawn(object):
    ...
    def monster(self, name,x,y,z, spawntime):
        self.cret.append(chr(61) + chr(len(name)) + name + struct.pack("<bbI", x, y, spawntime).decode('latin-1'))
        
    def npc(self, name,x,y,z, spawntime):
        self.cret.append(chr(62) + chr(len(name)) + name + struct.pack("<bbI", x, y, spawntime).decode('latin-1'))
    ...
    def gen(self, x,y,z,rx,ry, extras):
        if self.cret:
            ...
            code = struct.pack("<BHHBBB", 60, self.center[0], self.center[1], z, self.radius, len(self.cret)).decode('latin-1') # opCode + positionX + positionY + positionZ + amount of creatures + radius
