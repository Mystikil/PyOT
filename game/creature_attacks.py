...
from game.functions import getSpectators
...
class CreatureAttacks(object):
    ...
    def onHit(self, by, dmg, type, effect=None):
        ...
        if dmg:
            ...
            # Spectators should also see the damage text
            for spectator in getSpectators(self.position, ignore=(by, self)):
                stream = spectator.packet()
                stream.message(spectator.player, "", MSG_DAMAGE_OTHERS, color=textColor, value=-1 * dmg, pos=self.position)
    ...
    def onHeal(self, by, amount):
        if self.data["healthmax"] != self.data["health"]:
            ...
            for spectator in getSpectators(self.position, ignore=(by, self)):
                stream = spectator.packet()
                stream.message(spectator.player, "", MSG_HEALED_OTHERS, color=COLOR_GREEN, value=amount, pos=self.position)
                stream.send(spectator)
        else:
            return False
