...
class Monster(Creature):
    ...
    def onDeath(self):
            ...
            if not isSummon and not self.lastDamagers or self.getLastDamager() != self.master:
                ...
                for loot in drops:
                    ...
                    if lenLoot == 2:
                        ...
                        rarity.apply_rarity_to_item(ritem)
        ...
        for tile_item in tile.getItems():
            if tile_item.itemId in SMALLSPLASHES or tile_item.itemId in FULLSPLASHES:
                tile.removeItem(tile_item)
        ...
        if not isSummon and self.lastDamagers and self.getLastDamager().isPlayer() and self.getLastDamager() != self.master:
            ...
            reward = getattr(self, "exp_reward", self.base._experience)
            if attackerParty and attackerParty.shareExperience and attackerParty.checkShareExperience():
                for member in attackerParty.members:
                    if member.data["stamina"] or config.noStaminaNoExp == False:
                        exp = (reward / len(attackerParty.members)) * config.partyExperienceFactor
                ...
                if self.getLastDamager().data["stamina"] or config.noStaminaNoExp == False:
                    self.getLastDamager().modifyExperience(reward * self.getLastDamager().getExperienceRate())

                    if reward >= self.getLastDamager().data["level"]:
    ...
    def targetCheck(self, targets=None):
        ...
        for player in targets:
            if player.hasGroupFlag("IGNORED_BY_CREATURES"):
                continue
...
class MonsterBase(object):
    ...
    def spawn(self, position, place=True, spawnTime=None, spawnDelay=0.1, radius=5, radiusTo=None, monster=None, check=False):
        ...
        tile = position.getTile() if place else None
        if place and not tile:
        ...
        modules = self.ai_modules if self.ai_modules else (AttackAI,)
        monster.ai = MonsterAI(monster, modules)
        monster_rank.apply_rank(monster)

        if not monster.alive:
            monster.data = monster.base.data.copy()
            monster.alive = True

        if monster.clientId() not in allCreatures:
        ...
        if place and stackpos is not None and stackpos < 10:
    ...
    def ranks(self, **kwargs):
        """Configure rank promotion chances for this monster base."""
        for name, chance in kwargs.items():
            rank = name
            if not isinstance(rank, monster_rank.MonsterRank):
                rank = monster_rank.MonsterRank[name.upper()]
            self.rank_chances[rank] = chance
        return self
    def prepare(self):
        self.prepared = True
        if not self._loot:
