...
class Spell(object):
    ...
    def doEffect(self):
        ...
        def spellCallback(creature, strength=None, **k):
            ...
            if targetType in (TARGET_TARGET, TARGET_TARGETSELF, TARGET_TARGETONLY):
                if creature.target:
                    target = creature.target
                    if targetType == TARGET_TARGETONLY:
                        targetType = TARGET_TARGET
                elif targetType == TARGET_TARGET and self.targetArea:  # if no target but area still cast the spell
                    targetType = TARGET_AREA  # Cast as an area spell defined by targetArea
                elif targetType == TARGET_TARGET and not self.targetArea:
                    return False
                elif targetType == TARGET_TARGETONLY:
            ...
            if creature.isPlayer():
                ...
                if creature.getSkull() == SKULL_BLACK and config.blackSkullDisableAreaSpells and targetType == TARGET_AREA:
            ...
            if targetType in (TARGET_TARGET, TARGET_TARGETSELF) and target and target != creature and self.shootEffect:
                creature.shoot(creature.position, target.position, self.shootEffect)
                
            if targetType != TARGET_AREA:
            ...
            if targetType == TARGET_AREA:
                area = self.targetArea(caster=creature) if isinstance(self.targetArea, collections.abc.Callable) else self.targetArea
