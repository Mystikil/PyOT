...
import game.player, game.creature, game.npc, game.monster, game.spell, game.party, game.vocation
...
def loader(timer):
    ...
    class _CallLaterHandle:
        __slots__ = ('_handle', '_active')

        def __init__(self, handle, active):
            self._handle = handle
            self._active = active

        def cancel(self):
            builtins.ioloop_ins.remove_timeout(self._handle)
    ...
    def _call_later(delay, callback, *args, **kwargs):
        """Schedule a callback and provide an interface compatible with
        Twisted's ``callLater``.``"""
        ...
        handle = builtins.ioloop_ins.call_later(delay, _wrapper)

        return _CallLaterHandle(handle, active)

    def _remove_timeout(handle):
        if isinstance(handle, _CallLaterHandle):
            builtins.ioloop_ins.remove_timeout(handle._handle)
        else:
            builtins.ioloop_ins.remove_timeout(handle)

    builtins.call_later = _call_later

    builtins.call_at = builtins.ioloop_ins.call_at
    builtins.remove_timeout = _remove_timeout
    ...
    # Determine map format and prepare sectors
    otbm_path = os.path.join(config.dataDirectory, config.mapDirectory, 'map.otbm')
    world_sec_dir = os.path.join(config.dataDirectory, 'world', 'sec')
    if os.path.isdir(world_sec_dir):
        sec_dir = world_sec_dir
    else:
        sec_dir = os.path.join(config.dataDirectory, config.mapDirectory)
    print("> > Using sector directory: %s" % sec_dir)
    sectors_exist = bool(glob.glob(os.path.join(sec_dir, '*.sec')))
    map_format = 'sec'
    if os.path.exists(otbm_path) and not config.preferSecFormat:
        if not sectors_exist:
            print("> > Converting OTBM map...", end=' ')
            game.otbm_loader.convert_otbm_to_sec(otbm_path, sec_dir)
            print("%40s" % _txtColor("\t[DONE]", "blue"))
        map_format = 'otbm'
    print("> > Map format loaded: %s" % map_format)
    ...
    if config.loadEntierMap:
        ...
        begin = time.time()
        files = glob.glob(os.path.join(sec_dir, '*.sec'))
