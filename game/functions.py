...
def sendFloatingText(player, target, amount, isHeal=False):
    """Send a floating damage/heal text to a player using an extended opcode."""
    if not player or not player.client:
        return

    color = game.const.COLOR_GREEN if isHeal else game.const.COLOR_RED

    data = {
        'cid': getattr(target, 'cid', 0),
        'amount': int(amount),
        'color': color,
        'heal': bool(isHeal)
    }
