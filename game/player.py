...
class Player(PlayerTalking, PlayerAttacks, Creature): # Creature last.
    def __init__(self, client, data):
        ...
        data["skills"] = {
            SKILL_FIST: data.get('fist', config.defaultSkillLevel),
            SKILL_SWORD: data.get('sword', config.defaultSkillLevel),
            SKILL_CLUB: data.get('club', config.defaultSkillLevel),
            SKILL_AXE: data.get('axe', config.defaultSkillLevel),
            SKILL_DISTANCE: data.get('distance', config.defaultSkillLevel),
            SKILL_SHIELD: data.get('shield', config.defaultSkillLevel),
            SKILL_FISH: data.get('fishing', config.defaultSkillLevel),
            SKILL_ALCHEMY: data.get('alchemy', config.defaultSkillLevel),
            SKILL_DISCIPLINE: data.get('discipline', config.defaultSkillLevel),
            SKILL_DEXTERITY: data.get('dexterity', config.defaultSkillLevel),
            SKILL_SPELLWEAVING: data.get('spellweaving', config.defaultSkillLevel),
            SKILL_HEAVY_WEAPONS: data.get('heavy_weapons', config.defaultSkillLevel)
        }
        data["skill_tries"] =  {
            SKILL_FIST: data.get('fist_tries', 0),
            SKILL_SWORD: data.get('sword_tries', 0),
            SKILL_CLUB: data.get('club_tries', 0),
            SKILL_AXE: data.get('axe_tries', 0),
            SKILL_DISTANCE: data.get('distance_tries', 0),
            SKILL_SHIELD: data.get('shield_tries', 0),
            SKILL_FISH: data.get('fishing_tries', 0),
            SKILL_ALCHEMY: data.get('alchemy_tries', 0),
            SKILL_DISCIPLINE: data.get('discipline_tries', 0),
            SKILL_DEXTERITY: data.get('dexterity_tries', 0),
            SKILL_SPELLWEAVING: data.get('spellweaving_tries', 0),
            SKILL_HEAVY_WEAPONS: data.get('heavy_weapons_tries', 0)
        }
        ...
        # Compatibility: formula may accept manaspent only or also the strength constant
        ml_sig = inspect.signature(config.magicLevelFromManaFormula)
        const = getattr(config, "_strength_constant")
        try:
            if len(ml_sig.parameters) == 1:
                self.data["maglevel"] = int(config.magicLevelFromManaFormula(self.data["manaspent"]))
            else:
                self.data["maglevel"] = int(config.magicLevelFromManaFormula(self.data["manaspent"], const))
        except TypeError:
            # Fall back to trying both signatures if inspection was wrong
            try:
                self.data["maglevel"] = int(config.magicLevelFromManaFormula(self.data["manaspent"], const))
            except TypeError:
                self.data["maglevel"] = int(config.magicLevelFromManaFormula(self.data["manaspent"]))
    ...
    def description(self, isSelf=False):
        if isSelf:
            output = _l(self, "You see yourself. You are an adventurer.")
            output += " Your health is %d Your mana is %d Your current regeneration is %.2f%%" % (
                self.data.get("health", 0),
                self.data.get("mana", 0),
                0.50,
            )
        else:
            ### TODO: We can drop sex prefixes now that we need to hardcode them anyway!
            if self.data["sex"] == 1:
                output = _lc(self, 'look_at_male', "You see %(name)s (Level %(level)d). %(prefix)s is an adventurer.") % {"name": self.name(),
                                                "level": self.data["level"],
                                                "prefix": _l(self, self.sexPrefix())}
            else:
                output = _lc(self, 'look_at_female', "You see %(name)s (Level %(level)d). %(prefix)s is an adventurer.") % {"name": self.name(),
                                                "level": self.data["level"],
                                                "prefix": _l(self, self.sexPrefix())}
        return output
    ...
    def pong(self):
        stream = self.packet(0x1E)
        if getattr(config, 'pingIncludeTimestamp', False):
            stream.uint64(int(time.time() * 1000))
        stream.send(self.client)
    ...
    def setLevel(self, level, send=True):
        ...
        if oldLevel != level:
            ...
            self.data["healthmax"] = level * 5 + 145
            self.data["manamax"] = level * 5 - 5
            self.data["capacity"] = (level * 10 + 390) * 100

            if self.data["manamax"] < config.minMana:
                self.data["manamax"] = config.minMana
                print("[WARNING] Player %s (ID:%d) has mana max below minimum." % (self.name(), self.data["id"]))
    ...
    def modifySpentMana(self, mana, refresh=False):
        ...
        if mana > 0:
            while self.data["manaspent"] > int(config.magicLevelFormula(maglevel)):
                modify += 1
                maglevel += 1
    ...
    def addSkillLevel(self, skill, levels):
        ...
        # Update goals
        goal = config.skillFormula(self.skills[skill])
    ...
    def soulGain(self):
        ...
        if self.doingSoulGain > time.time():
            ...
            gainTime = 120 * self.getRegainRate()
    ...
    def sendModalWindow(self, title, message, buttons, choices=None, enterButton=0, escapeButton=1, callback=None):
        """Display a modal dialog if supported, otherwise fall back to a message."""
        if self.client.version >= 960:
            stream = self.packet()
            self.windowTextId += 1
            stream.modalDialog(self, self.windowTextId, title, message,
                               buttons, choices or [], enterButton, escapeButton)
            stream.send(self.client)
            if callback:
                self.setWindowHandler(self.windowTextId, callback)
            return self.windowTextId
        else:
            # Older clients do not support modal dialogs, use a normal message.
            self.windowMessage(f"{title}\n{message}")
            return 0
    ...
    def arrowUpContainer(self, openId):
        ...
        if bagFound.parent:
            ...
            res = game.scriptsystem.get('close').run(thing=bagFound, creature=self, index=openId)
            if res == False:
                return False
    ...
    def onDeath(self):
        ...
        if loseRate:
            ...
            for skill in range(SKILL_FIRST, SKILL_LAST):
                ...
                tries = config.totalSkillFormula(self.data["skills"][skill]) + self.getSkillAttempts(skill)
                ...
                level = int(config.skillTriesToLevel(tries))

                # Previous level skill tries.
                prevTries = int(config.totalSkillFormula(level-1))
    ...
    def _saveQuery(self, force=False):
        ...
        if self.saveSkills or force:
            ...
            # TODO: Custom skills.
            extraQuery += ", s.`fist` = %d, s.`fist_tries` = %d, s.`sword` = %d, s.`sword_tries` = %d, s.`club` = %d, s.`club_tries` = %d, s.`axe` = %d, s.`axe_tries` = %d, s.`distance` = %d, s.`distance_tries` = %d, s.`shield` = %d, s.`shield_tries` = %d, s.`fishing` = %d, s.`fishing_tries` = %d, s.`alchemy` = %d, s.`alchemy_tries` = %d, s.`discipline` = %d, s.`discipline_tries` = %d, s.`dexterity` = %d, s.`dexterity_tries` = %d, s.`spellweaving` = %d, s.`spellweaving_tries` = %d, s.`heavy_weapons` = %d, s.`heavy_weapons_tries` = %d" % (
                self.data["skills"][SKILL_FIST], self.data["skill_tries"][SKILL_FIST],
                self.data["skills"][SKILL_SWORD], self.data["skill_tries"][SKILL_SWORD],
                self.data["skills"][SKILL_CLUB], self.data["skill_tries"][SKILL_CLUB],
                self.data["skills"][SKILL_AXE], self.data["skill_tries"][SKILL_AXE],
                self.data["skills"][SKILL_DISTANCE], self.data["skill_tries"][SKILL_DISTANCE],
                self.data["skills"][SKILL_SHIELD], self.data["skill_tries"][SKILL_SHIELD],
                self.data["skills"][SKILL_FISH], self.data["skill_tries"][SKILL_FISH],
                self.data["skills"][SKILL_ALCHEMY], self.data["skill_tries"][SKILL_ALCHEMY],
                self.data["skills"][SKILL_DISCIPLINE], self.data["skill_tries"][SKILL_DISCIPLINE],
                self.data["skills"][SKILL_DEXTERITY], self.data["skill_tries"][SKILL_DEXTERITY],
                self.data["skills"][SKILL_SPELLWEAVING], self.data["skill_tries"][SKILL_SPELLWEAVING],
                self.data["skills"][SKILL_HEAVY_WEAPONS], self.data["skill_tries"][SKILL_HEAVY_WEAPONS]
            )
            self.saveSkills = False
    ...
    def popDepotItem(self, depotId, itemId):
        """Remove and return a single item instance from the depot."""
        depot = self.getDepot(depotId)
        if not depot:
            return None
    ...
    def calculateStrengthScore(self):
        return game.strength.calculateStrengthScore(self)
    ...
    def openMarket(self, marketId=0):
        ...
        if self.client.version >= 944:
            ...
            if self.marketDepotId not in self.depotMarketCache:
    ...
    def createMarketOffer(self, type, itemId, amount, price, anonymous=0):
        ...
        if type == 1 and amount == 1:
            itm = self.popDepotItem(self.marketDepotId, itemId)
            if itm and getattr(itm, 'serial', None):
                offer.itemSerial = itm.serial
                from tornado.ioloop import IOLoop
                IOLoop.instance().add_callback(game.item.saveItem, itm)
        if anonymous:
    ...
    def getAccountGroupFlags(self, default={}):
        try:
            return game.functions.groups[self.data["account_group_id"]][1]
        except:
            print("Warning: AccountGroupID %d doesnt exist!" % self.data["account_group_id"])
            return default

    def hasGroupFlag(self, flag):
        group_flags = self.getGroupFlags([])
        account_flags = self.getAccountGroupFlags([])

        return flag in group_flags or flag in account_flags

    def hasGroupFlags(self, *flags):
        g = self.getGroupFlags([])
        ag = self.getAccountGroupFlags([])

        for flag in flags:
            if flag not in g and flag not in ag:
                return False
    ...
    def sendExtendedJSONOpcode(self, opcode, data, max_size=65000):
        """Send extended opcode using JSON encoding with automatic chunking."""
        if not (self.client and self.getOSType() >= 0x0A):
            return

        payload = otjson.dumps(data)
        if len(payload) <= max_size:
            self.sendExtendedOpcode(opcode, payload)
            return

        self.sendExtendedOpcode(opcode, 'S' + payload[:max_size])
        pos = max_size
        while pos + max_size < len(payload):
            self.sendExtendedOpcode(opcode, 'P' + payload[pos:pos + max_size])
            pos += max_size
        self.sendExtendedOpcode(opcode, 'E' + payload[pos:])
