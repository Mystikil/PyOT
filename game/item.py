...
# generator yielding unique serial identifiers for items
def _serial_gen():
    while True:
        yield uuid.uuid4().hex

_next_serial = _serial_gen().__next__
### Item ###
class Item(object):
    ...
    def __init__(self, itemId, count=1, actions=None, **kwargs):
        ...
        # Prepare a generic tooltip for modern clients
        try:
            self.tooltip = self.description().strip()
        except Exception:
            self.tooltip = ""
    ...
    def description(self, player=None):
        ...
        if player and (not position or position.x == 0xFFFF or player.inRange(position, 1, 1)): # If position ain't set/known, we're usually in a trade situation and we should show it.
            ...
            # Append rarity info if present
            if getattr(self, "rarity_description", None):
                desc = self.rarity_description
                if player and player.data.get("group_id", 1) >= config.adminGroupId:
                    serial = getattr(self, "rarity_serial", None)
                    if serial:
                        desc = f"{desc} (#{serial})"
                extra += "\n%s" % desc
    ...
    def place(self, position, creature=None):
        ...
        if not isinstance(position, (Position, StackPosition)):
            # allow tuples or objects with x/y/z attributes
            if hasattr(position, 'x') and hasattr(position, 'y') and hasattr(position, 'z'):
                position = Position(position.x, position.y, position.z, getattr(position, 'instanceId', 0))
            elif isinstance(position, (list, tuple)) and len(position) >= 3:
                instance = position[3] if len(position) > 3 else 0
                position = Position(position[0], position[1], position[2], instance)
            else:
                raise AssertionError("position must be a Position-like object")
    ...
    def __setstate__(self, state):
        ...
        # Ensure tooltip is available for items loaded from storage
...
def _applyRarityMultipliers(item):
    """Adjust numeric stats based on item rarity."""
...
def loadItems():
    ...
    def _decoder(item):
        ...
        item["rarity"] = int(item.get("rarity", Rarity.JUNK))
        _applyRarityMultipliers(item)
        id = item['id']
        cid = item.get('cid')
...
@gen.coroutine
def saveItem(item):
    """Persist item data indexed by its serial."""
...
def deleteItem(serial):
    """Remove serialized item entry."""
    yield sql.runOperation("DELETE FROM items WHERE `serial`=%s", serial)
