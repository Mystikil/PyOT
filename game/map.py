...
# Directory containing compiled map sectors
_world_sec_dir = os.path.join(config.dataDirectory, "world", "sec")
if os.path.isdir(_world_sec_dir):
    SECTOR_DIR = _world_sec_dir
else:
    SECTOR_DIR = os.path.join(config.dataDirectory, config.mapDirectory)
print("> > Using sector directory: %s" % SECTOR_DIR)
...
def load(sectorX, sectorY, instanceId, sectorSum, verbose=True):
    ...
    try:
        sec_path = os.path.join(SECTOR_DIR, "%s%d.%d.sec" % (instances.get(instanceId, ''), sectorX, sectorY))
