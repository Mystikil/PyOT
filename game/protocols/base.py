...
class BasePacket(TibiaPacket):
    """Common packet routines shared by all protocol versions.

    The ``max*`` attributes define various limits that older client
    versions may impose.  When a limit is exceeded the protocol code
    transparently falls back to a safe default so the client does not
    crash or display invalid data.
    """

    maxKnownCreatures = 1300
    maxOutfits = 29
    maxMounts = 25
    # Highest item client id that can be sent to the client. Newer
    # clients support the full 16 bit range so use a high default here.
    maxItemId = 0xFFFF
    # Highest magic effect id that the client can handle.  Effects with
    # an id above this value will be replaced with ``EFFECT_POFF`` when
    # sent to the player.
    maxEffectId = 0xFF
    protocolEnums = {}
    _ok_ = []
    ...
    protocolEnums['_MSG_HEALED'] = protocolEnums['_MSG_EVENT_DEFAULT']
    protocolEnums['_MSG_HEALED_OTHERS'] = protocolEnums['_MSG_EVENT_DEFAULT']
    ...
    def magicEffect(self, pos, type):
        if type > self.maxEffectId:
            type = game.const.EFFECT_POFF
    ...
    def item(self, item, count=None):
        ...
        # Older clients cannot handle item ids above a certain value. When an
        # item exceeds the supported id range send a generic replacement so the
        # client does not crash or display a missing sprite.
        if cid > self.maxItemId:
            if item.solid:
                cid = 100
        ...
        # Send tooltip for modern clients if available
        if self.player and self.player.client.version >= 1321:
            self.string(getattr(item, 'tooltip', ''))
    ...
    def modalDialog(self, player, dialogId, title, message, buttons, choices=None, enterButton=0, escapeButton=1, priority=False):
        """Send a modal dialog with buttons and choice list."""
        if choices is None:
            choices = []
        ...
        if player.client.version >= 960:
            ...
            self.string(message)
            self.uint8(len(buttons))
            for bid, btext in buttons:
                self.string(btext)
                self.uint8(bid)

            self.uint8(len(choices))
            for cid, ctext in choices:
                self.string(ctext)
                self.uint8(cid)

            if player.client.version > 970:
                self.uint8(escapeButton)
                self.uint8(enterButton)
            else:
                self.uint8(enterButton)
                self.uint8(escapeButton)

            self.uint8(1 if priority else 0)
        else:
            pass  # TODO fallback for older clients
...
class BaseProtocol(object):
    ...
    def handleLookAt(self, player, packet):
        ...
        # Client and server use identical stack ordering (ground = 0). The
        # original conversion to a reversed index caused look operations on
        # walls and signs to return the ground description. Simply use the
        # provided stack position.
    ...
    @packet(0xF9)
    def handleModalDialogAnswer(self, player, packet):
        windowId = packet.uint32()
        button = packet.uint8()
        choice = packet.uint8()

        try:
            cb = player.windowHandlers[windowId]
        except KeyError:
            return
        ...
        try:
            ...
            del player.windowHandlers[windowId]
    ...
    def handlePing(self, player, packet):
        ts = None
        # Old clients may not send a timestamp
        if len(packet.data) - packet.pos >= 8:
            ts = packet.uint64()
            player.ping = max(0, int(time.time() * 1000) - ts)
