...
class CreatureMovement(object):
    ...
    def teleport(self, position, force=False):
        """Teleport creature to ``position`` while cleaning any queued moves."""

        # Cancel queued movement and actions first to avoid client desync
        try:
            self.stopAction()
        except Exception:
            pass
        try:
            ...
            pass
        if hasattr(self, "lastClientMove"):
            ...
            if thing.isItem():
                ...
                if teledest:
                    if not isPlayer:
                        return False
                    try:
                        # Use the current instance when teleporting and avoid
                        # passing it as the 'force' argument. This prevented
                        # the solid tile check from running and could place the
                        # creature inside blocking objects, leaving it unable
                        # to move afterwards.
                        self.teleport(
                            Position(
                                teledest[0],
