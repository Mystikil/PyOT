...
from game.item import Item, loadItem, deleteItem
from game.functions import loadPlayerById
import config, sql

Markets = {}

class Offer(object):
    def __init__(self, playerId, itemId, price, expire, amount, type=0, itemSerial=None):
        self.id = 0
    ...
    @gen.coroutine
    def insert(self):
        self.id = yield sql.runOperationLastId("INSERT INTO `market_offers`(`world_id`, `market_id`, `player_id`, `item_id`, `item_serial`, `amount`, `created`, `price`, `anonymous`, `type`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", config.worldId, self.marketId, self.playerId, self.itemId, self.itemSerial, self.amount, self.expire-config.marketOfferExpire, self.price, 1 if self.playerName == "Anonymous" else 0, self.type)
    ...
    def save(self):
        if not self.id:
            ...
            sql.runOperation("UPDATE market_offers SET `player_id` = %s, `item_id` = %s, `item_serial`=%s, `amount` = %s, `price` = %s, `type` = %s WHERE `id` = %s", self.playerId, self.itemId, self.itemSerial, self.amount, self.price, self.type, self.id)
    ...
    def expireOffer(self):
        ...
        if self.type == MARKET_OFFER_BUY:
            ...
            if self.itemSerial:
                item = yield loadItem(self.itemSerial)
                depot.append(item)
                yield deleteItem(self.itemSerial)
            else:
...
def load():
    ...
    x = yield sql.runQuery("SELECT mo.`id`, mo.`market_id`, mo.`player_id`, mo.`item_id`, mo.`item_serial`, mo.`amount`, mo.`created`, mo.`price`, mo.`anonymous`, mo.`type`, (SELECT `name` FROM players p WHERE p.`id` = mo.`player_id`) as `player_name` FROM `market_offers` mo WHERE mo.`world_id` = %s AND mo.`type` != 0", config.worldId)
    for entry in (x):
        if not entry["market_id"] in Markets:
            Markets[entry["market_id"]] = Market(entry["market_id"])
        market = Markets[entry["market_id"]]
        offer = Offer(entry["player_id"], entry["item_id"], entry["price"], entry["created"]+config.marketOfferExpire, entry["amount"], entry["type"], entry.get("item_serial"))
