...
class LoginProtocol(protocolbase.TibiaProtocol):
    ...
    def onFirstPacket(self, packet):
        ...
        # Ensure enough data is present for the version field.
        if packet.pos + 2 > len(packet.data):
            print("Invalid login packet: missing version")
            self.loseConnection()
            return
        try:
            ...
            if version >= 972:
                if packet.pos + 5 > len(packet.data):
                    raise struct.error
            ...
            if not len(packet.data) or v != 0: # RSA needs to decrypt just fine, so we get the data, and the first byte should be 0
                ...
                return
            # Set the XTEA key
            if packet.pos + 16 > len(packet.data):
                raise struct.error
            ...
            print("Invalid login packet: truncated data")
            self.loseConnection()
            return
