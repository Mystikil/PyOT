...
class GameProtocol(protocolbase.TibiaProtocol):
    ...
    def onFirstPacket(self, packet):
        ...
        if packetType == 0x0A and not self.ready:
            ...
            account = yield sql.runQuery(
                "SELECT `id`,`language`, `group_id` AS `account_group_id` "
                "FROM `accounts` WHERE `name` = %s AND `password` = SHA1(CONCAT(`salt`, %s))",
                username, password)
            ...
            character = yield sql.runQuery(
                "SELECT p.`id`,p.`name`,p.`world_id`,p.`group_id`,p.`account_id`,p.`health`,p.`mana`,p.`soul`,p.`manaspent`,p.`experience`,p.`posx`,p.`posy`,p.`posz`,p.`instanceId`,p.`sex`,p.`looktype`,p.`lookhead`,p.`lookbody`,p.`looklegs`,p.`lookfeet`,p.`lookaddons`,p.`lookmount`,p.`town_id`,p.`skull`,p.`stamina`, p.`storage`, p.`inventory`, p.`depot`, p.`conditions`, s.`fist`,s.`fist_tries`,s.`sword`,s.`sword_tries`,s.`club`,s.`club_tries`,s.`axe`,s.`axe_tries`,s.`distance`,s.`distance_tries`,s.`shield`,s.`shield_tries`,s.`fishing`, s.`fishing_tries`, g.`guild_id`, g.`guild_rank`, p.`balance` "
                "FROM `players` AS `p` LEFT JOIN player_skills AS `s` ON p.`id` = s.`player_id` "
            ...
            character['account_group_id'] = account['account_group_id']
            ...
            if player:
                self.player = player
                self.player.data["account_group_id"] = account["account_group_id"]
    ...
    def _send_ping(self):
        if self.player and self.ready:
            self.player.pong()
            interval = getattr(config, 'pingInterval', 30)
            self._ping_call = call_later(interval, self._send_ping)
