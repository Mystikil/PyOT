...
def onUseWith(creature, thing, position, onThing, onPosition, **k):
    ...
    required_level = potion.get("level")
    if required_level and creature.data["level"] < required_level:
        creature.say("Only players of level %d or above may drink this fluid." % required_level, MSG_SPEAK_MONSTER_SAY)
        return False
